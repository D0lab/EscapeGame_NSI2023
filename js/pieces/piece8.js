...
var sortir = false;
   ...
   function JeClique(objet){
	...
	function DansListe(element) {
		...
		if(element == 'boutonrouge'){			
			...
			explosionslieux.push(" "+element+" (p8) ")
		...
		if(element == 'porte' || element == 'boutonrouge' || element == 'moniteur' || element == 'serrure' || element == 'keypad' || element == 'ecran'){
			flag = 1;
		}
   ...
   function AddTrousse(objet){
	   ...
	   for(var i=0; i<objets_obtenus.length;i++){
		...
		if(objet == 'serrure' || objet == 'moniteur' || objet == 'keypad'){
			flag = 1;
		}
