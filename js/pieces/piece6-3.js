...
$(document).ready(function()	{
   ...
   var objets_trousse = ["tournevis"];
   var objets_obtenus = ["tournevis"];
   var sortir = false;
   var flag = 0;
   ...
   function JeClique(objet){
	...
	function DansListe(element) {
		...
		if(element == 'boutonrouge'){
			...
			explosionslieux.push(" "+element+" (p6-3) ")
   ...
   function AddTrousse(objet){
	   ...
	   for(var i=0; i<objets_obtenus.length;i++){
		...
		if(objet == 'ventilation' && objets_trousse.includes('tournevis') == false){
			$("#trousse").html($("#trousse").html()+'<br />'+"INACCESSIBLE"); 
		}
		
		else if(objet == 'ventilation' && objets_trousse.includes('tournevis') == true){
			$("#trousse").html($("#trousse").html()+'<br />'+"OUVERTURE");
			document.location.href="../pieces/piece6-2.html"; 
		}
	...
	function ParIciLaSortief(){
		sortir = true;	
		document.location.href="../pieces/piece7.html"; 
	}
	
	
	
	var piecescheck = JSON.parse(localStorage.getItem("check"));
	if (piecescheck.indexOf(" 6-3 ") === -1) {
		piecescheck.push(" 6-3 ")
	}	
